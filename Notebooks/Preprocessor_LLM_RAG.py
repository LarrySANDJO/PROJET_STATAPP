...
class PreprocessLLM_RAG:
    def __init__(self, label_cols, freq_cols, ordinal_cols, binary_cols, scale_cols, Fraud_col=None):
    ...
    def verbalize_row_rag(self, row):
        lines = []
        ...
        for col in self.label_cols:
            lines.append(f"{col} : {row[col]}")
        if (self.Fraud_col != None) & (self.Fraud_col in pd.DataFrame([row]).columns):
            lines.append(f"etat de Fraude : {row[self.Fraud_col]}")
        
        return "\n".join(lines)
    
    def transform_rag(self, df, id_col=None):
        outputs = []
        for idx, row in df.iterrows():
            obs_id = row[id_col] if id_col else idx
            text = self.verbalize_row_rag(row)
